...
export const useAdminRecentActivities = () => {
  ...
  const fetchActivities = async () => {
    try {
      ...
      const [usersResponse, paymentRequestsResponse, groupsResponse, productsResponse] = await Promise.all([
        api.getAllUsers(),
      ...
      const paymentRequests = paymentRequestsResponse.success ? paymentRequestsResponse.data.requests : [];
      ...
      paymentRequests
        ...
        .sort((a, b) => new Date(b.fechaAprobacion || b.fechaSolicitud).getTime() - new Date(a.fechaAprobacion || a.fechaSolicitud).getTime())
