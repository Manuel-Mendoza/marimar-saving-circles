...
const GroupDetailsView: React.FC<GroupDetailsViewProps> = ({ groupId, onBack }) => {
  ...
  const getStatusBadge = (estado: string) => {
    const statusConfig = {
      ...
      'EN_MARCHA': { variant: 'default' as const, icon: TrendingUp, color: 'text-blue-600' },
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-600 mt-1">ID: {data.group.id} • {data.group.duracionMeses} meses de duración</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Estado actual: {data.group.estado === 'EN_MARCHA'
        ...
        <CardContent>
          ...
          <div className="flex justify-between text-sm text-gray-600 mt-2">
            <span>Inicio: {data.group.fechaInicio ? new Date(data.group.fechaInicio).toLocaleDateString("es-ES") : 'No iniciado'}</span>
            <span>{Math.round(getProgressValue())}% completado</span>
            <span>Fin: {data.group.fechaFinal ? new Date(data.group.fechaFinal).toLocaleDateString("es-ES") : 'No definido'}</span>
          </div>
        </CardContent>
      </Card>
      ...
      <Tabs defaultValue="members" className="space-y-4">
        <TabsList className="grid w-full grid-cols-3">
          ...
          <TabsTrigger value="contributions">Contribuciones ({data.stats.totalContributions})</TabsTrigger>
        ...
        <TabsContent value="members" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Lista completa de participantes ordenados por posición</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {data.members.map((member) => (
                    <TableRow key={member.id}>
                      ...
                      <TableCell>
                        <div>
                          <p className="font-medium">{member.user.nombre} {member.user.apellido}</p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-lg font-bold">
                      {data.contributions.reduce((sum, c) => sum + (c.estado === 'CONFIRMADO' ? c.monto : 0), 0).toFixed(2)}
                    </p>
              ...
              <Table>
                ...
                <TableBody>
                  {data.members.map((member) => {
                    ...
                    const memberContributions = data.contributions.filter(c => c.userId === member.user.id);
                    const confirmedPayments = memberContributions.filter(c => c.estado === 'CONFIRMADO');
                    const pendingPayments = memberContributions.filter(c => c.estado === 'PENDIENTE');
                      ...
                      <TableRow key={member.id}>
                        <TableCell>
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <p className="font-medium text-sm">{member.user.nombre} {member.user.apellido}</p>
                              <p className="text-xs text-gray-500">{member.user.correoElectronico}</p>
                            </div>
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <div className="space-y-1">
                            ...
                            <p className="text-xs text-gray-500">
                              {confirmedPayments.length === expectedPayments ? 'Al día' :
                               confirmedPayments.length > expectedPayments ? 'Adelantado' : 'Atrasado'}
                            </p>
                          </div>
                        </TableCell>
                        <TableCell>
                          <span className="font-medium">{totalPaid.toFixed(2)} {member.monedaPago}</span>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <div className="text-sm">
                            {lastPayment ? (
                              <div>
                                <p className="font-medium">{lastPayment.monto.toFixed(2)} {lastPayment.moneda}</p>
                                <p className="text-xs text-gray-500">
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <Avatar className="w-6 h-6">
                            <AvatarFallback className="text-xs">
                              {delivery.user.nombre[0]}{delivery.user.apellido[0]}
                            </AvatarFallback>
                          </Avatar>
                          <span className="text-sm">{delivery.user.nombre} {delivery.user.apellido}</span>
                        </div>
                      </TableCell>
